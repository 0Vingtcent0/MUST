...
def _remove_boundary_cells(mask):
    ...
    try:
        ...
        print(f"Warning: cannot unpack mask shape {mask.shape}: {e}")
        return mask

    bd = np.ones((W, H), dtype=mask.dtype)
    if W > 4 and H > 4:
        bd[2:W - 2, 2:H - 2] = 0
    else:
        bd = np.zeros((W, H), dtype=mask.dtype)
    ...
    try:
        ...
        print(f"Warning: relabel_sequential failed with error {e}, fallback to original mask.")
        new_label = mask
        return new_label
    return new_label
